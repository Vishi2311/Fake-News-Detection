...
def detect_relationship(
    ...
    claim_words = important_tokens(claim)
    evidence_words = important_tokens(evidence_text)

    entities_claim = extract_entities(claim)
    entities_evidence = extract_entities(evidence_text)

    events_claim = extract_event_groups(claim)
    events_evidence = extract_event_groups(evidence_text)

    word_score = overlap_score(claim_words, evidence_words)
    entity_score = entity_overlap(entities_claim, entities_evidence)
    event_score = event_overlap(events_claim, events_evidence)

    years_claim = extract_years(claim)
    years_evidence = extract_years(evidence_text)
    ...
    if years_claim:
        year_match = (
            1.0
            if years_claim.intersection(years_evidence)
            else 0.0
        )

    # ========================================================
    # CLAIM / EVIDENCE POLARITY
    # ========================================================

    claim_negated = has_negation(claim)
    evidence_negated = has_negation(evidence_text)

    # One side negative and the other positive indicates a
    # possible contradiction. It is accepted only when the
    # entities/events and wording also overlap sufficiently.
    polarity_conflict = (
    ...
    if strong_event_match:
        return {
            "relationship": "STRONG_EVENT_MATCH",
            "supporting": True,
            "contradicting": False,
            "word_score": word_score,
            "entity_score": entity_score,
    ...
    if claim_type == "FUTURE_OR_PLAN":
        ...
        if (
            years_claim
            and not years_claim.intersection(years_evidence)
        ):
            return {
                "relationship": "RELATED_ONLY",
                "supporting": False,
                "contradicting": False,
                "word_score": word_score,
                "entity_score": entity_score,
